function [d,si,h]=abfload(fn,varargin)
  ...
  %                                 only information on erroneous input will
  %                                 be displayed)
  % << OUTPUT VARIABLES <<<
  ...
  %
  %   GNU Octave's adaptation by Tzvetomir Tzvetanov (t.tzvetanov@neuropsyphy.io
  %   or tzvet@qq.com), 2022-03-30/31

  % tt 20220330 check if it's Octave
  ...
  % tt 20220330
  dispif(doDispInfo,sprintf('   -- opening file %s resulted in fid=%d (>0 is OK), messg=%s (empty is OK)\n',fn,fid, messg));
  if fid == -1
    error(messg);
  end
  ...
  fileSz=ftell(fid);
  fseek(fid,0,'bof');
  dispif(doDispInfo,sprintf('   -- file size %d\n',fileSz)); % tt 20220330
  ...
  dispif(doDispInfo,sprintf('   -- type of ABF version (fFileSignature):    %s\n',fFileSignature)); % tt 20220330
  % rewind
  fseek(fid,0,'bof');
  % transpose
  ...
  if h.fFileVersionNumber>=2
    ...
    %char(BigString)' % tt 20220331 print the string with directory/file name
    if isoctave BigString=native2unicode(BigString); end % tt 20220330 octave uses UTF-8 
    % extract path and name of protocol file (excluding drive letter):
    % - find the first occurrence of a backslash in BigString
    %tmpIx1=strfind(lower(char(BigString)'),'\'); % tt 20220330 not important
    tmpIx1=(strfind(lower(char(BigString)'),'\'))(1); % tt 20220330
    ...
    for i=1:ADCSection.llNumEntries
      ...
      % tt 20220330 -> octave fread() does not have the 'bitn' read format, thus
      % code needs small readjustments (though seems this var is useless)
      if isoctave && sum( strcmp( 'bEnabledDuringPN', fieldnames(ADCsec(i)) ) )>0 
        if doDispInfo fprintf('    -- changing ADCsec(i).bEnabledDuringPN from %d',ADCsec(i).bEnabledDuringPN); end
        ADCsec(i).bEnabledDuringPN = bitget(ADCsec(i).bEnabledDuringPN,1);
        if doDispInfo fprintf(' to %d\n',ADCsec(i).bEnabledDuringPN); end
      end % tt 20220330 - end
    ...
    % tt 20220330 -> octave fread() does not have the 'bitn' read format, thus
    % code needs small readjustments
    if isoctave && sum( strcmp( 'bEnableFileCompression', fieldnames(ProtocolSec) ) )>0 
      if doDispInfo
        fprintf('    -- changing ProtocolSec.bEnableFileCompression from %d',...
        ProtocolSec.bEnableFileCompression);
      end
      ProtocolSec.bEnableFileCompression = bitget(ProtocolSec.bEnableFileCompression,1);
      if doDispInfo fprintf(' to %d\n',ProtocolSec.bEnableFileCompression); end
    end % tt 20220330 end
    ...
    if EpochPerDACSection.llNumEntries>0
      ...
      for k=1:numel(uniqueAO)
        ...
        if ~isoctave % tt 20220331 differences in setdiff() matlab & octave
          fieldNm=setdiff(fieldnames(EPDsec),'nDACNum','stable');
        else
          [c,ia]=setdiff(fieldnames(EPDsec),'nDACNum');
          fieldNm=fieldnames(EPDsec)(sort(ia));
        end % tt 20220331 end
  ...
  switch h.nOperationMode
    ...
    case 3
      ...
      dispif(doDispInfo,['sampling interval: ' num2str(h.si,'%5.0f') ' us']);
...
function headPar=define_header(fileSig)
  switch fileSig
   ...
   case 'ABF2'
     ...
     global isoctave; % tt 20220331
     if isoctave % tt 20220331
       headPar(2,:) = {'fFileVersionNumber',4,'uint8=>int',[-1 -1 -1 -1]};
     end
  end
end
...
function ProtocolInfo=define_ProtocolInfo
  ...
  global isoctave % tt 20220331
  if isoctave % tt 20220331 -> needs to extract bit1
    ProtocolInfo(3,:) = {'bEnableFileCompression','uint8',1};
  end
end
...
function ADCInfo=define_ADCInfo
  ...
  global isoctave % tt 20220331
  if isoctave % tt 20220331 -> needs to extract bit1
    ADCInfo(24,:) = {'bEnabledDuringPN','uint8',1};
  end
end
